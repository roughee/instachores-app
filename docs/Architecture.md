...
- **Poller**: while the document is visible, `sync()` every 30 s; also immediately on `visibilitychange` to visible, on `online`, and after every outbox flush. Back off to 2 minutes after three consecutive failures, reset on success. The cursor is the largest `loggedAt` seen, stored with the snapshot. Every 10th `sync()`, any sync the poller did not schedule itself (`syncNow()`, a tab becoming visible, coming back online), and every sync while the snapshot still has no household (a resumed session syncs at once on `start()` in that case), re-run `bootstrap` instead of `events.since` and merge household, tasks and rewards too, so a hand edit to the sheet or a session that connected before the sheet was seeded reaches the app on its own (issue #52). A sync is always a poll or a refresh, never both.
...
- **Build id (issue #45).** `vite.buildId.ts`'s `resolveBuildId` stamps a build id at build time: the first 7 characters of `GITHUB_SHA` when CI sets it (`deploy.yml`/`preview.yml`), otherwise a UTC dev timestamp (`dev-20260909T1200`). `vite.config.ts` puts it in `define.__BUILD_ID__`, which reaches both the app bundle (`src/buildId.ts`, imported by `SettingsScreen.vue`'s "App build" row) and the service worker, since `define` is the one option vite-plugin-pwa shares between the app and `injectManifest` worker builds.
- **`strategies: 'injectManifest'`, not `generateSW`**: a hand-written `src/sw.ts` (`srcDir: 'src'`, `filename: 'sw.ts'`) precaches the shell (`precacheAndRoute(self.__WB_MANIFEST)`), falls every navigation back to `index.html` (`NavigationRoute` + `createHandlerBoundToURL`, replacing `generateSW`'s `navigateFallback`), keeps the same `NetworkOnly` rule for `script.google.com` and `CacheFirst` for fonts and icons, and never calls `self.skipWaiting()` on its own -- only on the `SKIP_WAITING` message `usePwa.ts`'s `reload()` sends, matching the prompt-style update flow above. This is what lets it also answer a `postMessage({ type: 'version' })` handshake with its own `__BUILD_ID__` over a `MessageChannel` port, which `usePwa.ts`'s `readServiceWorkerVersion` reads for Settings' "Service worker" row -- the ACTIVE worker's id, "Not registered" with no controller (e.g. Vitest or a non-secure context). Read once per page load, so a stale worker keeps showing its old id until the update toast is accepted and the page actually reloads.
