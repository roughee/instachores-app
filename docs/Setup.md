...
4. In the function picker at the top of the editor, choose `setupTemplate_` and click Run. Authorize the script when prompted (it is your own script acting on your own sheet). This creates the five tabs with header rows and plain-text formatting; it is safe to re-run.
...
1. Before deploying, open the Apps Script editor, select `test_` in the function picker and click Run. Check **View > Logs** for `PASS` on every scenario and a final `ALL PASS (n)`.
