...
- **`docs/adr/`**: read ADRs that touch the area you're about to work in. The first three planned are Firestore over Supabase, append-only events, and Vue over React (Plan §7.2).
- **`docs/Plan.md`**: the product plan. §3 is the task catalog, §5 the pages and their acceptance criteria, §6 the architecture, §7 the process.
...
│   ├── Plan.md
│   ├── adr/
│   │   ├── 0001-firestore-over-supabase.md
...
> _Contradicts ADR-0002 (append-only events), but worth reopening because…_
